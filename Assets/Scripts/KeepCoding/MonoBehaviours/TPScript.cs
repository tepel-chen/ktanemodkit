...
namespace KModkit
{
    ...
    public abstract class TPScript<TModule> : MonoBehaviour, ITP where TModule : ModuleBase
    {
        ...
        protected static bool IsMatch(string input, string pattern, bool lenient = true,
            RegexOptions options = RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)
        {
            return Regex.IsMatch(input.NullCheck("You cannot do a Regular Expression check where the message is null."), lenient ? @"^\s*" + pattern.NullCheck("You cannot do a Regular Expression check where the expression is null.") + @"\s*$" : pattern.NullCheck("You cannot do a Regular Expression check where the expression is null."), options);
        }
        ...
        {
            return AppendIfNotNullOrEmpty("trywaitcancel", time, message);
        }
        ...
        /// <param name="message">The message to send.</param>
        /// <returns>A formatted string for Twitch Plays.</returns>
        protected static string SendToChat(string message)
        {
        ...
        /// <param name="message">The message to send.</param>
        /// <returns>A formatted string for Twitch Plays.</returns>
        protected static string Detonate(float? time = null, string message = null)
        {
            return AppendIfNotNullOrEmpty("detonate", time, message);
        }
        ...
        {
            return AppendIfNotNullOrEmpty("skiptime", seconds);
        }
