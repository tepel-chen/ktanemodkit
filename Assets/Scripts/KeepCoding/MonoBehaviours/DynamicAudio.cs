...
namespace KModkit
{
    ...
    public sealed class DynamicAudio : MonoBehaviour
    {
        ...
        public int? GameVolume
        {
            get
            {
                return Application.isEditor ? 100 : IsMusicLayer ? Game.PlayerSettings.MusicVolume : Game.PlayerSettings.SFXVolume;
            }
        }
        ...
        public static explicit operator AudioSource(DynamicAudio dynamicAudio)
        {
            return dynamicAudio.AudioSource;
        }
...
#pragma warning restore IDE0051 // Remove unused private members
        ...
        {
            GetRoutineMethod(_fade.Count)(volume, time);
        }
        ...
        /// <param name="time">The time in the audio it should start playing at.</param>
        /// <param name="pitch">The pitch of the sound.</param>
        public void Play(string sound, bool loop = false, int priority = 0, float delay = 0, float pitch = 1,
            float time = 0, float volume = 1)
        {
            var clip = AudioClips.FirstOrDefault(c => c.name == sound);
            if (clip == null)
                throw new MissingComponentException(String.Format(
                    "There is no sound effect named \"{0}\". List of audio clips from the prefab: {1}", sound, AudioClips.UnwrapToString()));
            Play(clip, loop, priority, delay, pitch, time, volume);
        }
        ...
        private IEnumerator UpdateVolume()
        {
            ...
            while (true)
            {
                AudioSource.volume = Volume * (int)GameVolume / 100f;
                yield return null;
            }
        }

        private Action<float, float> GetRoutineMethod(int length)
        {
            if (length == 0)
                return (i, j) => _fade.Start(i, j, false);
            return _fade.Restart;
        }
    }
}
